...
def test_rotate():
    ...
    assert np.array_equal(c.cmap[0], [0, 0, 0]), "Array 0th element didn't shift out (%s)" % str(c.cmap[0])
    assert np.array_equal(c.cmap[1], [1, 1, 1]), "Array 1st element didn't shift in (%s)" % str(c.cmap[1])
